...
import {DoctorProfile,Booking,Slot} from "@/types/doctor"
...
export default function DoctorsPage() {
  ...
  const verifyPaymentMutation = useMutation({
    mutationFn: async (payload: { bookingId: string; razorpayPaymentId?: string; razorpayOrderId?: string; razorpaySignature?: string; mockPaymentSuccess?: boolean }) => {
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-8">
        <div>
          ...
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mt-1">Consult verified specialists or check your booked schedules</p>
        </div>

        {/* Tab Controls */}
        <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200/50 self-start sm:self-center">
          <button onClick={() => setActiveTab("find")} className={`px-5 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === "find" ? "bg-slate-900 text-white shadow-sm" : "text-slate-500 hover:text-slate-800"}`}>
            Find Doctors
          </button>
          <button
            ...
            {active.length > 0 && <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-emerald-500 rounded-full animate-ping" />}
            My Consultations
          </button>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-6 animate-in fade-in duration-200">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-white p-4 border border-slate-100 rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.01)]">
            <input
              ...
              className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 placeholder:text-slate-500 focus:outline-none focus:ring-1 focus:ring-slate-900"
            />

            <select value={specialization} onChange={(e) => setSpecialization(e.target.value)} className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:ring-1 focus:ring-slate-900">
            ...
            <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 focus:outline-none focus:ring-1 focus:ring-slate-900">
          ...
          {doctorsLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {sortedDoctors.map((doc) => (
                <div key={doc._id} className="bg-white border border-slate-100 rounded-[24px] shadow-[0_4px_25px_rgba(0,0,0,0.01)] p-6 flex flex-col gap-4 hover:-translate-y-0.5 transition-all duration-200">
                  <div className="flex items-center gap-3.5">
                    <div className="w-12 h-12 rounded-[16px] bg-gradient-to-br from-slate-900 to-slate-700 text-white flex items-center justify-center font-black text-lg">{doc.name.charAt(0).toUpperCase()}</div>
                    <div>
                  ...
                  <p className="text-xs text-slate-500 line-clamp-2 leading-relaxed h-8">{doc.about || "No profile bio provided yet."}</p>
                  ...
                  <button onClick={() => setSelectedDoctor(doc)} className="w-full py-3 bg-slate-900 hover:bg-slate-800 text-white text-xs font-black rounded-xl transition-all">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {active.map((booking) => (
                  <div key={booking._id} className="bg-emerald-50/40 border border-emerald-100 rounded-[24px] p-6 flex flex-col gap-4">
                    ...
                    <button onClick={() => router.push(`/doctors/consultation/${booking._id}`)} className="w-full py-3 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black rounded-xl transition-all shadow-md shadow-emerald-600/10">
                ...
                {upcoming.map((booking) => {
                  ...
                  return (
                    <div key={booking._id} className="bg-white border border-slate-100 rounded-[24px] p-6 flex flex-col gap-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-slate-900 text-white flex items-center justify-center font-bold">{booking.doctorProfileId?.name?.charAt(0).toUpperCase() || "D"}</div>
                        <div>
                  ...
                  return (
                    <div key={booking._id} className="bg-white border border-slate-100 rounded-[24px] p-6 flex flex-col gap-4">
                      <div className="flex items-center justify-between border-b border-slate-50 pb-2">
                        ...
                        <span className="text-[9px] font-black uppercase bg-slate-100 px-2 py-0.5 rounded text-slate-400">{booking.status}</span>
                      </div>
                      <div className="text-[11px] text-slate-500 flex flex-col gap-1.5">
                        <div className="flex justify-between">
                          <span>Date:</span> <span className="font-bold text-slate-700">{bookingDate}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Slot:</span>{" "}
                      ...
                      <button onClick={() => router.push(`/doctors/consultation/${booking._id}`)} className="w-full py-2 border border-slate-200 hover:bg-slate-50 text-slate-700 text-xs font-bold rounded-lg transition-colors mt-2">
                        View Chat History
                      </button>
                    </div>
                  );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-[28px] shadow-2xl p-6 sm:p-8 w-full max-w-2xl border border-slate-100 max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-start justify-between border-b border-slate-100 pb-4 mb-6">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-[16px] bg-slate-900 text-white flex items-center justify-center font-black text-lg">{selectedDoctor.name.charAt(0).toUpperCase()}</div>
                <div>
              ...
              <button onClick={() => setSelectedDoctor(null)} className="w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500 transition-colors">
                ✕
              </button>
            </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    {selectedDoctor.qualifications.map((q) => (
                      <span key={q} className="bg-white border border-slate-200 text-slate-700 px-2 py-0.5 rounded text-[10px] font-bold">
                        {q}
                      </span>
                    ))}
                  </div>
            ...
            {/* Time Slots grid */}
            <div>
              <span className="text-[11px] font-black text-slate-400 uppercase tracking-wider ml-1">2. Select Available Slot (10:00 AM - 10:00 PM)</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-[28px] shadow-2xl p-6 sm:p-8 w-full max-w-md border border-slate-100 animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex flex-col gap-3">
              <button type="button" onClick={handlePayment} disabled={verifyPaymentMutation.isPending} className="w-full py-4 bg-slate-900 hover:bg-slate-800 text-white font-black text-sm rounded-[16px] shadow-lg transition-all">
                {verifyPaymentMutation.isPending ? "Processing..." : "💳 Pay Now (Razorpay)"}
              </button>

              {/* Mock pay option for easy developer test */}
              <button type="button" onClick={handleMockPay} disabled={verifyPaymentMutation.isPending} className="w-full py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl transition-all">
                ⚡ Bypass Payment (Mock Success)
              </button>

              <button type="button" disabled={verifyPaymentMutation.isPending} onClick={() => setCheckoutBooking(null)} className="w-full py-2.5 text-slate-400 hover:text-slate-700 font-bold text-xs transition-colors mt-1">
                Cancel Booking
              </button>
            </div>
          </div>
