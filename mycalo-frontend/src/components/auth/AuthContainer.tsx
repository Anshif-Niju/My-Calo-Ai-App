    ...
    return (
      <div className="space-y-5">
        ...
        <div className="relative my-2 flex items-center justify-center">
          ...
          <span className="absolute bg-white px-3 text-[9px] font-bold text-slate-400 uppercase tracking-widest">
            or sign in with email
          </span>
        </div>
        <form onSubmit={handleLoginSubmit(onLoginSubmit)} className="space-y-3.5">
          <div>
            <label className="block text-[10px] font-bold text-slate-700 uppercase tracking-wider mb-1 ml-0.5">
              Email address
            </label>
            <input
              type="email"
            ...
            />
            {loginErrors.email && (
              <p className="text-[10px] font-semibold text-red-500 mt-1 ml-0.5">
          ...
          <div>
            <div className="flex justify-between items-center mb-1 ml-0.5">
              ...
              <a
                ...
                className="text-[10px] font-bold text-slate-400 hover:text-slate-950 transition-colors"
              >
                Forgot password?
              </a>
            </div>
    ...
    return (
      <div className="space-y-4">
            ...
            <div className="space-y-3 animate-fade-in">
              ...
              <div className="pt-2">
                <button
                  ...
                  className="w-full h-11 bg-slate-950 hover:bg-slate-800 text-white font-bold rounded-xl transition-all flex items-center justify-center gap-2 text-xs cursor-pointer"
                >
                  Continue
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" />
          ...
          {registerStep === 2 && (
            <div className="space-y-3.5 animate-fade-in">
              <div>
                ...
                <input type="text" {...regRegister("name")} placeholder="Name" className="w-full h-11 px-4 rounded-xl border border-slate-200 bg-white text-slate-900 font-medium placeholder:text-slate-400 focus:border-slate-950 focus:ring-2 focus:ring-slate-950/10 transition-all outline-none text-sm" />
              ...
              <div>
                ...
                <input type="email" {...regRegister("email")} placeholder="Enter your email" className="w-full h-11 px-4 rounded-xl border border-slate-200 bg-white text-slate-900 font-medium placeholder:text-slate-400 focus:border-slate-950 focus:ring-2 focus:ring-slate-950/10 transition-all outline-none text-sm" />
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <div className="relative">
                    <select value={watchCountryCode} onChange={(e) => setRegValue("countryCode", e.target.value, { shouldValidate: true })} className="h-11 pl-3 pr-8 rounded-xl border border-slate-200 bg-white text-slate-900 font-bold text-xs appearance-none focus:border-slate-950 focus:ring-2 focus:ring-slate-950/10 transition-all outline-none cursor-pointer">
                      {COUNTRY_CODES.map((c) => (<option key={c.code} value={c.code}>{c.flag} {c.code}</option>))}
                    </select>
                    <svg className="w-3 h-3 text-slate-400 absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" /></svg>
                  </div>
                  <input type="tel" {...regRegister("phone")} placeholder="Phone Number" className="flex-1 h-11 px-4 rounded-xl border border-slate-200 bg-white text-slate-900 font-medium placeholder:text-slate-400 focus:border-slate-950 focus:ring-2 focus:ring-slate-950/10 transition-all outline-none text-sm" />
                </div>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={handlePrevStep} className="flex-1 h-11 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold rounded-xl transition-all flex items-center justify-center text-xs cursor-pointer">Back</button>
                <button type="button" onClick={handleNextStep} className="flex-1 h-11 bg-slate-950 hover:bg-slate-800 text-white font-bold rounded-xl transition-all flex items-center justify-center text-xs cursor-pointer">Next</button>
              </div>
            </div>
          )}
          {registerStep === 3 && (
            <div className="space-y-3.5 animate-fade-in">
              <div>
                ...
                <div className="relative">
                  <input type={showRegPassword ? "text" : "password"} {...regRegister("password")} placeholder="••••••••" className="w-full h-11 pl-4 pr-10 rounded-xl border border-slate-200 bg-white text-slate-900 font-medium placeholder:text-slate-400 focus:border-slate-950 focus:ring-2 focus:ring-slate-950/10 transition-all outline-none text-sm" />
                  <button type="button" onClick={() => setShowRegPassword(!showRegPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 transition-colors cursor-pointer">
                    {showRegPassword ? <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg> : <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.773 3.162 10.065 7.498a10.523 10.523 0 01-4.293 5.774M6.228 6.228L3 3m3.228 3.228l3.65 3.65m7.894 7.894L21 21m-3.228-3.228l-3.65-3.65m0 0a3 3 0 10-4.243-4.243m4.242 4.242L9.88 9.88" /></svg>}
                  </button>
                </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showRegConfirmPassword ? "text" : "password"} {...regRegister("confirmPassword")} placeholder="••••••••" className="w-full h-11 pl-4 pr-10 rounded-xl border border-slate-200 bg-white text-slate-900 font-medium placeholder:text-slate-400 focus:border-slate-950 focus:ring-2 focus:ring-slate-950/10 transition-all outline-none text-sm" />
                  <button type="button" onClick={() => setShowRegConfirmPassword(!showRegConfirmPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-slate-600 transition-colors cursor-pointer">
                    {showRegConfirmPassword ? <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg> : <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.773 3.162 10.065 7.498a10.523 10.523 0 01-4.293 5.774M6.228 6.228L3 3m3.228 3.228l3.65 3.65m7.894 7.894L21 21m-3.228-3.228l-3.65-3.65m0 0a3 3 0 10-4.243-4.243m4.242 4.242L9.88 9.88" /></svg>}
                  </button>
                </div>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" disabled={registerMutation.isPending} onClick={handlePrevStep} className="flex-1 h-11 bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-bold rounded-xl transition-all flex items-center justify-center text-xs cursor-pointer disabled:opacity-50">Back</button>
                <button type="submit" disabled={registerMutation.isPending} className="flex-1 h-11 bg-slate-950 hover:bg-slate-800 text-white font-bold rounded-xl transition-all shadow-[0_4px_12px_rgba(15,23,42,0.15)] active:scale-[0.98] disabled:opacity-75 flex items-center justify-center text-xs cursor-pointer">
        ...
        <p className="text-center text-xs font-medium text-slate-500 pt-1">
          ...
          <button onClick={() => navigateTo("login")} className="font-bold text-slate-950 hover:underline cursor-pointer">Log in here</button>
        </p>
      </div>
    );
  };
  ...
  return (
    <div className="w-full flex justify-center items-center py-4">
      <div className="hidden md:flex w-full max-w-[1000px] h-[660px] bg-white rounded-3xl shadow-[0_20px_50px_rgba(15,23,42,0.06)] border border-slate-100 relative overflow-hidden flex-row transition-all duration-300">
        <div className={`absolute top-0 bottom-0 w-1/2 bg-slate-950 text-white z-20 flex flex-col justify-between p-12 transition-all duration-700 ease-in-out ${activeTab === "login" ? "left-0 transform translate-x-0" : "left-0 transform translate-x-full"}`}>
          ...
          <div className="relative py-4 flex flex-col items-center justify-center">
            ...
            <div className="w-72 bg-slate-900/60 backdrop-blur-md border border-white/10 rounded-2xl p-4 shadow-[0_15px_30px_rgba(0,0,0,0.3)] space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2 text-center text-[10px] font-bold text-slate-400">
                <div className="p-1.5 bg-slate-800/40 rounded-lg"><p className="text-emerald-400">180g</p><p className="text-[8px] text-slate-500">Carbs</p></div>
                <div className="p-1.5 bg-slate-800/40 rounded-lg"><p className="text-sky-400">110g</p><p className="text-[8px] text-slate-500">Protein</p></div>
                <div className="p-1.5 bg-slate-800/40 rounded-lg"><p className="text-amber-400">45g</p><p className="text-[8px] text-slate-500">Fat</p></div>
              </div>
            </div>
          </div>
          <div className="space-y-6">
            ...
            <ul className="space-y-2 text-xs font-semibold text-slate-300">
              <li className="flex items-center gap-2"><span className="w-5 h-5 bg-white/10 text-white rounded-full flex items-center justify-center shrink-0"><svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg></span>Real-time calorie & macro logging</li>
              <li className="flex items-center gap-2"><span className="w-5 h-5 bg-white/10 text-white rounded-full flex items-center justify-center shrink-0"><svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg></span>AI-powered personalized advice</li>
              <li className="flex items-center gap-2"><span className="w-5 h-5 bg-white/10 text-white rounded-full flex items-center justify-center shrink-0"><svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg></span>Smart analytics and weekly reports</li>
            </ul>
          </div>
        </div>
        <div className={`w-1/2 h-full flex flex-col justify-center px-12 md:px-14 absolute right-0 top-0 transition-all duration-700 ease-in-out ${activeTab === "login" ? "opacity-100 translate-x-0 pointer-events-auto z-10" : "opacity-0 translate-x-12 pointer-events-none z-0"}`}>
          {renderLoginFormFields()}
        </div>
        <div className={`w-1/2 h-full flex flex-col justify-center px-12 md:px-14 absolute left-0 top-0 transition-all duration-700 ease-in-out ${activeTab === "register" ? "opacity-100 translate-x-0 pointer-events-auto z-10" : "opacity-0 -translate-x-12 pointer-events-none z-0"}`}>
      ...
      <div className="md:hidden w-full max-w-[400px] flex flex-col items-center px-2">
        <div className="flex flex-col items-center mb-6">
          <div className="w-12 h-12 bg-slate-950 rounded-[18px] flex items-center justify-center mb-4 shadow-lg shadow-slate-900/20">
            <svg className="w-6 h-6 text-white" fill="currentColor" viewBox="0 0 24 24"><path d="M20 10c0-4.42-3.58-8-8-8s-8 3.58-8 8c0 4.42 3.58 8 8 8s8-3.58 8-8zM12 4c3.31 0 6 2.69 6 6s-2.69 6-6 6-6-2.69-6-6 2.69-6 6-6z" /></svg>
          </div>
        ...
        <div className="w-full bg-white p-6 sm:p-8 rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 relative z-20">
          {activeTab === "login" ? renderLoginFormFields() : renderRegisterFormFields()}
        </div>
      </div>
    </div>
  );
