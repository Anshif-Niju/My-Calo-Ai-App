...
export default function AdminDashboard() {
  ...
  const stats = [
    { title: "Total Users", value: data?.totalUsers || 0, icon: "👥", color: "text-emerald-700", bg: "bg-emerald-50/60", border: "border-emerald-100/60" },
    { title: "Total Doctors", value: data?.totalDoctors || 0, icon: "👨‍⚕️", color: "text-teal-700", bg: "bg-teal-50/60", border: "border-teal-100/60" },
    { title: "Appointments", value: data?.totalAppointments || 0, icon: "📅", color: "text-indigo-700", bg: "bg-indigo-50/60", border: "border-indigo-100/60" },
    { title: "Pending Verify", value: data?.pendingVerifications || 0, icon: "⏳", color: "text-amber-700", bg: "bg-amber-50/60", border: "border-amber-100/60" },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8">
            ...
            <div key={item.title} className="bg-white rounded-[24px] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 hover:-translate-y-1 transition-transform duration-300 group">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <h2 className="text-3xl sm:text-4xl font-black text-slate-900 mt-2 tracking-tight group-hover:scale-105 origin-left transition-transform">{item.value}</h2>
                </div>
                <div className={`w-12 h-12 rounded-[16px] border ${item.border} flex items-center justify-center text-xl ${item.bg} ${item.color}`}>{item.icon}</div>
              </div>
            </div>
          ))}
        </div>
            ...
            <div className="h-[250px] sm:h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={revenueData}>
                  ...
                  <Line type="monotone" dataKey="users" stroke="#059669" strokeWidth={4} dot={false} activeDot={{ r: 8, fill: "#059669", stroke: "#fff", strokeWidth: 3 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
        ...
        <div className="bg-white rounded-[32px] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
          <div className="flex items-center justify-between mb-6 ml-2 mr-2">
            ...
            <span className="px-3 py-1 bg-emerald-50 text-emerald-700 text-[11px] font-bold uppercase tracking-wider rounded-full">{data?.pendingVerifications || 0} Pending</span>
          </div>

          {/* Action Cards */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Link href="/admin/doctors" className="p-4 rounded-[20px] border border-slate-100 bg-slate-50 hover:bg-slate-100 transition-colors flex items-center justify-between group">
            ...
            <Link href="/admin/setting" className="p-4 rounded-[20px] border border-slate-100 bg-slate-50 hover:bg-slate-100 transition-colors flex items-center justify-between group">
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-[12px] bg-white border border-slate-200 flex items-center justify-center text-lg shadow-sm">⚙️</div>
                <div>
