  ...
  return (
    <div className="min-h-screen w-full bg-[#f8fafc] flex flex-col items-center justify-center p-4 selection:bg-slate-900 selection:text-white relative overflow-hidden">
      ...
      <main className="w-full max-w-[480px] bg-white/80 backdrop-blur-2xl p-10 sm:p-12 rounded-[40px] shadow-[0_20px_60px_rgb(0,0,0,0.05)] border border-white/40 flex flex-col items-center text-center relative z-10">
        ...
        <div className="inline-block px-4 py-1.5 rounded-full bg-slate-100 text-slate-600 text-xs font-bold tracking-widest uppercase mb-6">The Future of Health</div>
        ...
        <p className="text-base font-medium text-slate-500 mb-10 leading-relaxed">Your personal health architect. Track your daily nutrition, consult verified doctors, and achieve your goals with advanced AI.</p>

        {/* Action Buttons */}
        <div className="w-full flex flex-col gap-4">
          <Link href="/register" className="w-full h-[60px] bg-slate-950 hover:bg-slate-800 text-white font-bold rounded-[24px] transition-all shadow-[0_10px_20px_rgba(0,0,0,0.1)] active:scale-[0.98] flex items-center justify-center text-lg">
            Get Started Free
          </Link>

          <Link href="/login" className="w-full h-[60px] bg-white hover:bg-slate-50 text-slate-900 font-bold rounded-[24px] transition-all border-2 border-slate-100 active:scale-[0.98] flex items-center justify-center text-lg">
            Log In to Account
          </Link>
        </div>
      </main>
