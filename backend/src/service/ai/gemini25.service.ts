...
export const scanFoodWithGemini25 = async (imageBase64: string, mimeType: string) => {
  try {
    ...
    console.log("✅ Gemini 2.5 completed");
    return parseFoodResponse(result.response.text());
  } catch (error: any) {
    console.error("❌ Gemini 2.5 failed:", error?.message || error);
    throw new Error(`Gemini2.5: ${error?.message || "Unknown error"}`);
  }
};
