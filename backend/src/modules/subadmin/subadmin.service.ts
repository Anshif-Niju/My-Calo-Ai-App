...
export const getDashboardStats = async () => {
  const pending = await DoctorVerification.find({ verificationStatus: "pending" }).populate("userId", "name email phone profilePhoto createdAt").sort({ createdAt: -1 }).lean();
...
export const getVerificationDetail = async (doctorId: string) => {
  const verification = await DoctorVerification.findById(doctorId).populate("userId", "name email phone profilePhoto countryCode createdAt").lean();
...
export const getAllDoctors = async ({ page, limit, search }: { page: number; limit: number; search?: string }) => {
  ...
  const enriched = await Promise.all(
    doctorUsers.map(async (u) => {
      const [verification, profile] = await Promise.all([DoctorVerification.findOne({ userId: u._id }).lean(), DoctorProfile.findOne({ doctorId: u._id }).lean()]);
      return {
        ...u,
...
export const getDoctorDetail = async (userId: string) => {
  ...
  const [verification, profile] = await Promise.all([DoctorVerification.findOne({ userId }).lean(), DoctorProfile.findOne({ doctorId: userId }).lean()]);
...
export const hardDeleteDoctor = async (userId: string) => {
  ...
  await Promise.all([User.findByIdAndDelete(userId), DoctorVerification.findOneAndDelete({ userId }), DoctorProfile.findOneAndDelete({ doctorId: userId })]);
